...
2. 点上方 Tab 切换编辑区（一次只显示一种）：
   ...
   - **按键序列**：添加 / 编辑 / 排序步骤（可含等待、点击）
3. 按需设循环、热键、目标窗口（「取前台」可自动填入；同进程多窗口会带标题区分）
4. 点 **保存并应用**，再点 **开始**。多套配置可同时运行；用 **全部停止** 一键停掉
5. 默认 **ControlSend + 不激活**，后台推送按键，无需把目标窗口置前（部分游戏/特殊程序可能无效）

常用按键写法：`a`、`{Enter}`、`{Space}`、`{Tab}`、`{Esc}`、`{F1}`、`^c`（Ctrl+C）、`!{F4}`（Alt+F4）。

### 同一个客户端多开怎么配

只填 `exe` 是分不出多开的哪一个的（标题往往也一模一样），目标窗口区提供两种区分方式：

- **选窗口…**：列出该进程当前所有窗口，选中一个直接锁定句柄。最准，但客户端重启后句柄会变，需要重选。
- **窗口 #**：按窗口创建顺序取第 N 个（`0` = 自动挑一个没被其它配置占用的）。重启后依然有效，推荐长期使用。
