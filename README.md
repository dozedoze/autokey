...
- **多套配置**：左侧新建、复制、切换、删除多套方案；支持 Ctrl / Shift 多选后依次启动；选中窗口后在名称后以 `[进程 #序号]` 标识，不改动也不写入窗口标题
- 按进程名（`exe`）定位目标；多开时用「选择目标窗口…」锁定具体实例
- **同一客户端多开**：可锁定具体窗口句柄，或指定「第几个窗口」，多套配置各控一个互不抢占
- 开始 / 停止 / 暂停热键（支持界面内「捕捉」）
- **全局热键**：依次启动选中（默认 `Ctrl+F9`）、全部停止（默认 `F12`），可在界面捕捉修改
- 循环次数与间隔可配
...
4. 点 **保存并应用**，再点 **开始**。需要批量运行时，用 Ctrl / Shift 在左侧多选后点 **依次启动选中**（或按全局热键，默认 `Ctrl+F9`）；程序会等前一项执行完第一步再启动下一项，避免多个首步点击互相抢占。用 **全部停止**（或 `F12`）一键停掉
